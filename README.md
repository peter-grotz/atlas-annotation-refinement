...
Corrects atlas-propagated brain annotations against a small number of
manual ones, and determines when a correction is valid.

## Purpose

Accelerates generation of ground-truth neuroanatomic annotation: reference
regions for evaluating and improving registration to a chosen template.

Reverse registration — warping a template's parcellation onto each
specimen via the inverse of the specimen-to-template transform — produces
a complete annotation for every specimen at no per-specimen annotation
cost. Its boundaries follow template anatomy, not the anatomy present in
each acquisition, and carry systematic, structure-specific error: the same
deformation resolves the same structure the same way across specimens.

Two-stage, iterative process:

1. **Propagate.** Reverse-register the template parcellation onto a
   specimen. Zero-cost baseline annotation.
2. **Correct and reintroduce.** A small number of specimens are manually
   ...
   from the fit. Ground truth for a structure can then be supplied to the
   registration stage as an additional similarity term
   (`registration.MetricTerm`), constraining the deformation with anatomy
   that image intensity alone does not localize. Re-running propagation
   with the constrained registration improves label placement at the
   source, across the whole cohort, not only the corrected specimens.

Round *n*'s corrected annotations improve round *n*+1's registration. The
package provides propagation, correction, validation, and the
`MetricTerm` reintroduction mechanism. It does not run the loop
automatically — each round is scoped by an operator (*Operating model*).

The correction method cannot be fixed in advance. The two dominant failure
modes require near-opposite operations: over-coverage of a
lower-intensity margin is removed by an intensity threshold;
under-coverage of an interior with contrast on both boundaries requires
expansion followed by a contour or deformable method. Which mode applies
is a property of how the registration resolved that structure in that
cohort, not of the structure's identity — observable only after
propagation, and it can differ between two boundaries of the same
...
treatments in the same specimens.

One to five corrected specimens per structure is an order of magnitude
below what supervised segmentation requires. The problem is posed as
constrained program synthesis over a small operator space (thresholds,
morphological operations, deformable/contour methods), conditioned on a
measured error signature, checked against an exact criterion: agreement
with held-out ground truth.

## Operating model

The package supplies measurement and validation, not judgment.
...
stages run through the Python API.

### Result, one structure

Isocortex: Dice 0.905 (propagated) → 0.976, one free parameter fitted on
two independent annotations, transferring to a held-out specimen at
0.968. A second parameter (region-wise thresholding, background-facing
vs. white-matter-facing) changed the fitted score by 0.0005 against its
full search range and was discarded. Screening applied to all ten
specimens (two annotated, eight not) identified a 24% left–right volume
asymmetry on an unannotated specimen, attributed to registration rather
...
Python 3.10+. Registration requires ANTsPy. Characterization, analysis,
and review require NumPy, SciPy, and Matplotlib only.
...
`basis` — `pushed` (from the propagated label), `scratch`, or `refined`
(from a correction's output) — is not verifiable at intake and determines
admissibility: `pushed`/`scratch` are independent evidence; `refined` is
not, since its optimum is pinned to the parameter that produced it.
...
`loader` maps a specimen id to `(image, propagated_label, reference)`.
...
`atlas_refine.review` covers the specimens where it does not — typically
the majority, since a correction fitted on few specimens is applied to
the full cohort.
...
Neither establishes correctness — only implausibility, or its absence.
See `skills/visual-review/SKILL.md`.
...
<sub>Same rendering, ventricles: propagated (cyan) vs. manually corrected
(magenta). `axis 0 · 324` shows both lateral ventricles, where the
correction moves the boundary onto the fluid-filled lumen the propagated
label under- or over-reached.</sub>
...
- **Geometry.** Volumes carry explicit unit and coordinate frame;
  operations between them assert compatibility.
